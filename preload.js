...
var url = 'mongodb://allinwind.win:27017/takeout';
...
var insertSeller = (db) => {
  return db.collection('seller').insertOne(seller)
};

var insertGoods = (db) => {
  let p = goods.map(function (good) {
    db.collection('goods').insertOne(good)
  });
  return Promise.all(p)
};

var insertRatings = (db) => {
  let p = ratings.map(function (rating) {
...
var dropSeller = db => {
  return db.collection('seller').deleteMany({})
};

var dropGoods = db => {
  return db.collection('goods').deleteMany({})
};

var dropRatings = db => {
  return db.collection('ratings').deleteMany({})
};

MongoClient.connect(url, function (err, db) {

  dropSeller(db).then(_ => {

    return dropGoods(db)
  }).then(_ => {

    return dropRatings(db)
  }).then(_ => {

    return insertSeller(db).then(_ => {
      console.log('insert seller success');
      return insertGoods(db).then(_ => {
